...
const USERS = [
  { name: 'Officer John',       email: 'john@campus.edu',       role: 'OFFICER',             password: 'admin123'  },
  { name: 'Mike Tech',          email: 'mike@energy.edu',       role: 'ENERGY_TECHNICIAN',   password: 'pass123'   },
  { name: 'Alex NMC',           email: 'alex@energy.edu',       role: 'NMC_MEMBER',          password: 'pass123'   },
  { name: 'Tom Plumber',        email: 'tom@water.edu',         role: 'PLUMBING_SPECIALIST', password: 'pass123'   },
  { name: 'Irrigation Lead',    email: 'irr@water.edu',         role: 'IRRIGATION_MANAGER',  password: 'pass123'   },
  { name: 'Green Manager',      email: 'manager@waste.edu',     role: 'WASTE_MANAGER',       password: 'pass123'   },
  { name: 'Sanitation Officer', email: 'sanitation@waste.edu',  role: 'SANITATION_OFFICER',  password: 'pass123'   },
];
...
async function seed() {
  ...
  console.log('  Officer:             john@campus.edu       / admin123');
  console.log('  Energy Technician:   mike@energy.edu       / pass123');
  console.log('  NMC Member:          alex@energy.edu       / pass123');
  console.log('  Plumbing Specialist: tom@water.edu         / pass123');
  console.log('  Irrigation Manager:  irr@water.edu         / pass123');
  console.log('  Waste Manager:       manager@waste.edu     / pass123');
  console.log('  Sanitation Officer:  sanitation@waste.edu  / pass123');

  mongoose.connection.close();
}

seed().catch(e => { console.error('Seed failed:', e.message); mongoose.connection.close(); process.exit(1); });
