...
// Build daily buckets for a given resource type
function dailyBuckets(raw, type, days = 7) {
  const now = new Date();
  ...
  (raw || [])
    .filter(r => !type || r._id.type === type)
    .forEach(r => {
      const b = buckets.find(b => b.date === r._id.date);
      if (b) b.value += r.value;
    });
  return buckets;
}

// Build weekly (4-week) buckets for a given resource type
function weeklyBuckets(raw, type) {
  const now = new Date();
  ...
  (raw || [])
    .filter(r => !type || r._id.type === type)
    .forEach(r => {
      const w = weeks.find(w => r._id.date >= w.s && r._id.date <= w.e);
      if (w) w.value += r.value;
    });
  return weeks;
}

// Build combined chart data with all 3 resource types per date label
function buildCombinedChart(raw, view) {
  const now = new Date();
  if (view === 'weekly') {
    const days = 7;
    ...
    (raw || []).forEach(r => {
      const row = dates.find(d => d.date === r._id.date);
      if (row && r._id.type) row[r._id.type] = (row[r._id.type] || 0) + r.value;
    });
    return dates;
  } else {
    const weeks = Array.from({ length: 4 }, (_, i) => {
    ...
    (raw || []).forEach(r => {
      const w = weeks.find(w => r._id.date >= w.s && r._id.date <= w.e);
      if (w && r._id.type) w[r._id.type] = (w[r._id.type] || 0) + r.value;
    });
    return weeks;
  }
}
...
const CombinedTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '10px 14px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
      <p style={{ fontWeight: 700, fontSize: 11, color: '#64748b', marginBottom: 6 }}>{label}</p>
      {payload.map(p => (
        <div key={p.dataKey} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: p.color }} />
          <span style={{ fontSize: 11, color: '#475569' }}>{RESOURCE_CONFIG[p.dataKey]?.label || p.dataKey}</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#1e293b', marginLeft: 'auto', paddingLeft: 12 }}>
            {Number(p.value || 0).toLocaleString()} {RESOURCE_CONFIG[p.dataKey]?.unit}
          </span>
        </div>
      ))}
    </div>
...
// Individual resource chart card
const ResourceChart = ({ type, rawWeekly, rawMonthly, view }) => {
  const cfg = RESOURCE_CONFIG[type];
  const data = view === 'weekly'
    ? dailyBuckets(rawWeekly, type, 7)
    : weeklyBuckets(rawMonthly, type);
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className={`p-2 ${cfg.bgClass} rounded-xl`}>{cfg.icon}</div>
          <div>
            ...
            <p className="text-[10px] text-slate-400 uppercase tracking-wide font-bold">
              {view === 'weekly' ? 'Last 7 Days' : 'Last 4 Weeks'}
            </p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-lg font-bold text-slate-800">{fmt(Math.round(total))}</p>
          <p className="text-[10px] text-slate-400">{cfg.unit} total</p>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barSize={view === 'weekly' ? 18 : 28}>
            ...
            <Tooltip
              ...
              formatter={(v) => [fmt(Math.round(v)) + ' ' + cfg.unit, cfg.label]}
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        <div className="bg-slate-50 rounded-xl px-3 py-2 text-center">
          ...
          <p className="text-sm font-bold text-slate-700">{fmt(Math.round(peak))}</p>
        </div>
        <div className="bg-slate-50 rounded-xl px-3 py-2 text-center">
          <p className="text-[10px] text-slate-400 font-bold uppercase">Avg / Period</p>
          <p className="text-sm font-bold text-slate-700">{fmt(Math.round(avg))}</p>
        </div>
      </div>
    </div>
  );
...
const StrategicView = ({ stats, weekly, monthly, user, view, setView }) => {
  ...
  const rawForCombined = view === 'weekly' ? weekly : monthly;
  const combinedData   = useMemo(() => buildCombinedChart(rawForCombined, view), [rawForCombined, view]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { icon: <Zap size={20} />,      iconBg: 'bg-yellow-50 text-yellow-600',   label: 'Total Electricity', value: fmt(elecTotal),  unit: 'kWh'    },
          { icon: <Droplets size={20} />, iconBg: 'bg-blue-50 text-blue-600',       label: 'Total Water',       value: fmt(waterTotal), unit: 'Liters' },
          { icon: <Trash2 size={20} />,   iconBg: 'bg-emerald-50 text-emerald-600', label: 'Total Waste',       value: fmt(wasteTotal), unit: 'Kg'     },
        ].map((c, i) => (
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-800 flex items-center gap-2">
          ...
          {view === 'weekly' ? 'Weekly Breakdown — Last 7 Days' : 'Monthly Breakdown — Last 4 Weeks'}
        </h3>
      ...
      {/* Combined multi-resource line chart */}
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <p className="font-bold text-slate-800 text-sm flex items-center gap-2 mb-1">
          <TrendingUp size={16} className="text-emerald-600" />All Resources — Combined View
        </p>
        <p className="text-xs text-slate-400 mb-4">
          {view === 'weekly' ? 'Daily consumption totals per resource type' : 'Weekly consumption totals per resource type'}
        </p>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={combinedData}>
              ...
              <Legend
                ...
                formatter={(value) => RESOURCE_CONFIG[value]?.label || value}
              />
              {visibleTypes.includes(ResourceType.ELECTRICITY) && (
                <Line type="monotone" dataKey="ELECTRICITY" stroke="#EAB308" strokeWidth={2.5}
                  dot={{ r: 3, fill: '#EAB308' }} activeDot={{ r: 5 }} />
              )}
              {visibleTypes.includes(ResourceType.WATER) && (
                <Line type="monotone" dataKey="WATER" stroke="#3B82F6" strokeWidth={2.5}
                  dot={{ r: 3, fill: '#3B82F6' }} activeDot={{ r: 5 }} />
              )}
              {visibleTypes.includes(ResourceType.WASTE) && (
                <Line type="monotone" dataKey="WASTE" stroke="#10B981" strokeWidth={2.5}
      ...
      {/* Individual bar charts per resource */}
      <div className={`grid gap-4 ${
        visibleTypes.length === 1 ? 'grid-cols-1 max-w-sm' :
        visibleTypes.length === 2 ? 'grid-cols-1 md:grid-cols-2' :
            ...
            <div key={type} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-slate-800">
                  {s ? fmt(Math.round(s.total)) : '—'}{' '}
                  <span className="text-sm font-normal text-slate-400">{cfg.unit}</span>
                </p>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-slate-800 flex items-center gap-2">
            <ClipboardList size={18} className="text-emerald-600" />My Tickets — {cfg.label}
          </h3>
...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* AI Insights — strategic only */}
      {userLayer === 'STRATEGIC' && !loading && (
        <AIInsights stats={statsData.stats} />
      )}
